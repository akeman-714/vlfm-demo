...
if [ -n "${PIDS_ON_PORT}" ]; then
  for pid in ${PIDS_ON_PORT}; do
    ...
    if printf '%s' "${cmd}" | grep -Eq 'scripts/(cat_demo/web|dataset_tools/semantic_cat_web)\.py'; then
      echo ">>> Replacing old web.py on port ${SEMANTIC_CAT_WEB_PORT}: pid ${pid}"
...
python scripts/cat_demo/web.py
